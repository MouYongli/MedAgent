...
# File storage directory (data directory is under the backend root directory)
...
async def get_file(filename: str):
    """
    Get the content of the specified file for editing
    """
...
async def delete_file(filename: str):
    ...
    if not os.path.exists(file_path):
        raise HTTPException(status_code=404, detail="File not found")
    os.remove(file_path)
    return {"message": "Delete successful", "filename": filename}
