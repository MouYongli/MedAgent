...
def create_retrieval_scores(
    ...
    logger.info(
        "Starting retrieval evaluation with number of expected entries [{len(expected_retrieval)}] and number of actual entries [{len(actual_retrieval)}] ...")
    ...
    normalized_expected_retrieval, normalized_actual_retrieval = copy.deepcopy(expected_retrieval), copy.deepcopy(
        actual_retrieval)
    ...
    for sub in sorted(substrings, key=lambda x: len(x['er_lcs']), reverse=True):
        ...
        found, normalized_expected_retrieval[i_er].text, normalized_actual_retrieval[
            i_ar].text = find_and_remove_substring(normalized_expected_retrieval[i_er].text,
                                                   normalized_actual_retrieval[i_ar].text, lcs_er, lcs_ar)
        if found:
            ...
            logger.info(
                f" - Partial TP for expected: {len(lcs_er)} / {original_lengths_expected[i_er]} = {partial_tp_expected_value} --> Now at value {partial_tp_expected[i_er]} for the expected entry")
            logger.info(
                f" - Partial TP for retrieved: {len(lcs_ar)} / {original_lengths_actual[i_ar]} = {partial_tp_actual_value} --> Now at value {partial_tp_actual[i_ar]} for the retrieved entry")
        else:
    ...
    for i_er, er in enumerate(normalized_expected_retrieval):
        if len(er.text) > 0:
            ...
            logger.info(
                f"[Expected {i_er}]  partial TP: {partial_tp_expected[i_er]:.2f}; partial FN {partial_fn_expected[i_er]:.2f} (should be {(1 - partial_tp_expected[i_er]):.2f})")
    for i_ar, ar in enumerate(normalized_actual_retrieval):
        if len(ar.text) > 0:
            partial_fp_actual[i_ar] = len(ar.text) / original_lengths_actual[i_ar]
            logger.info(
                f"[Actual   {i_ar}]  partial TP: {partial_tp_actual[i_ar]:.2f}; partial FP {partial_fp_actual[i_ar]:.2f} (should be {(1 - partial_tp_actual[i_ar]):.2f})")
