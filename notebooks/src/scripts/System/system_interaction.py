...
from general.data_model.system_interactions import WorkflowSystem, ChatInteraction, GenerationResultEntry, \
    RetrievalEntry
...
def pose_question(backend_api_url, chat_id, question) -> Tuple[str, Optional[Any], float]:
    ...
    response.raise_for_status()
    system_answer, retrieval_result, response_latency = response.json()["response"], retrieval if (
                                                                                                      retrieval := response.json().get(
                                                                                                          "retrieval",
                                                                                                          [])) is not None else [], \
    response.json()["response_latency"]
...
def generate_stored_response(
        dbi: MongoDBInterface, wf_system: WorkflowSystem, chat: Optional[ChatInteraction], question: QuestionEntry,
        backend_api_url
    ...
    for retrieval_entry in retrieval:
        gl = dbi.document_to_guideline_metadata(
            dbi.get_entry(CollectionName.GUIDELINES, "awmf_register_number", retrieval_entry["guideline_id"]))
    ...
    retrieval_precision, retrieval_recall, retrieval_f1 = create_retrieval_scores(entry.question.expected_answers,
    ...
    entry.feedback.append(create_feedback_accuracy(
        create_accuracy_scores(expected_retrieval=entry.question.expected_answers, provided_answer=entry.answer)))
